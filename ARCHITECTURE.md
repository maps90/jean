...
- **Ports** (`ports.py`) — `SessionStore`, `TranscriptStore`, `ApprovalCoordinator`,
  `MaintenanceStore`, `ThreadLock`, `ChatSurface`. Structural (`Protocol`) interfaces,
  so adapters satisfy them by shape and tests can use plain fakes.
...
| `ports.py` | Protocol interfaces: `SessionStore`, `TranscriptStore`, `ApprovalCoordinator`, `MaintenanceStore`, `ThreadLock`, `ChatSurface` |
...
- `maintenance(job, last_run)` — one row per background job (`cleanup`), so the
  retention interval survives a restart and only one worker prunes per cycle.

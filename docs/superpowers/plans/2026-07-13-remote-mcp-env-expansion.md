...
def test_a_remote_servers_credential_comes_from_the_environment(monkeypatch):
    ...
    expanded = expand_config(
        {
            "type": "http",
            "url": "https://portico.internal.example/mcp",
    ...
    assert expanded["url"] == "https://portico.internal.example/mcp"
...
2. **the marketplace, `portico` skill:** rewrite it to call the `mcp__portico__*` tools instead of teaching the `curl .../mcp` + `python3 -c` JSON-RPC pattern. **Left as-is, the skill undoes this entire change** — jean will keep curling out of habit, and every curl is an approval click.

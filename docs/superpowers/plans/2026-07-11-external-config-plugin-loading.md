...
def test_load_plugin_manifest_parses_entries(tmp_path):
    ...
    p.write_text(json.dumps({"plugins": [
        {"marketplace": "git@github.com:example-org/skills.git", "plugin": "grafana", "ref": "main"},
    ]}))
...
async def test_missing_plugin_raises(tmp_path):
    ...
    with pytest.raises(RuntimeError):
        await r.resolve([PluginRef("git@github.com:example-org/skills.git", "elasticsearch", "main")])
```

...
Every one of those is a `curl` to `https://portico.internal.example/mcp` — hand-written
...
- **the marketplace, `portico` skill:** this is where the `curl https://portico…/mcp` pattern is
